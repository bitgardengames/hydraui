...
local HighlightPlayerMention = function(message)
        ...
        local FormatHighlight = function(mention)
                return format("|cFF%s%s|r", Color, mention)
        end

        local GetHyperlinkRanges = function(text)
                local ranges
                local search = 1

                while true do
                        local start = find(text, "|H", search, true)

                        if (not start) then
                                break
                        end

                        local finish = find(text, "|h", start + 2, true)
                        ...
                        ranges = ranges or {}
                        ranges[#ranges + 1] = {start, finish + 1}
                        search = finish + 2
                end
        ...
        local IsWithinHyperlink = function(ranges, index)
                if (not ranges) then
                        return false
                end

                for i = 1, #ranges do
                        local bound = ranges[i]

                        if (index >= bound[1]) and (index <= bound[2]) then
                                return true
                        end
                end
        ...
        local HasColorPrefix = function(source, index)
                local start = index - 10

                if (start < 1) then
                        return false
                end

                return (sub(source, start, start + 1) == "|c")
        end

        local Source = message

        message = gsub(Source, "()(@" .. PlayerNamePattern .. ")", function(index, mention)
                if PlayerNameWithRealmPattern then
                        local NextChar = sub(Source, index + #mention, index + #mention)
                ...
                return FormatHighlight(mention)
        end)

        if PlayerNameWithRealmPattern then
                message = gsub(message, "(@" .. PlayerNameWithRealmPattern .. ")", FormatHighlight)
        end

        if PlayerNameWithRealmPattern then
                Source = message

                local ranges = GetHyperlinkRanges(Source)

                message = gsub(Source, "()(" .. PlayerNameWithRealmPattern .. ")", function(index, mention)
                        if IsWithinHyperlink(ranges, index) then
                                return mention
                        end

                        local Prev = sub(Source, index - 1, index - 1)
                        ...
                        local NextChar = sub(Source, index + #mention, index + #mention)
                        local PrevIsWord = false
                        local NextIsWord = false

                        if (Prev ~= "") and (Prev ~= nil) then
                                if match(Prev, "[%w\128-\255%-]") and (not HasColorPrefix(Source, index)) then
                                        PrevIsWord = true
                                end
                        end

                        if (NextChar ~= "") and (NextChar ~= nil) then
                                NextIsWord = match(NextChar, "[%w\128-\255%-]") and true or false
                        end

                        if PrevIsWord or NextIsWord then
                                return mention
                        end
        ...
        message = gsub(Source, "()(" .. PlayerNamePattern .. ")", function(index, mention)
                ...
                if PlayerNameWithRealm then
                        local Combined = sub(Source, index, index + #PlayerNameWithRealm - 1)

                        if (Combined == PlayerNameWithRealm) then
                                return mention
                        end
                end
